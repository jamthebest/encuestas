...
@if ($Encuesta)
  <div class="btn-agregar" style="text-align: center; margin-top:3%">
    {{ link_to_route('VerPanelistas', 'Ver Panelistas', array($Encuesta->id), array('class' => 'btn btn-warning', 'style'=>'width:160px; height:160px; margin-right:2%; padding-top: 8%; margin-left:-77%')) }}
    {{ link_to_route('AsignarPanelistas', 'Asignar Panelistas', array($Encuesta->id), array('class' => 'btn btn-primary', 'style'=>'width:160px; height:160px; margin-right:2%; padding-top: 8%')) }}
    {{ link_to_route('Pagos.Index', 'Pagos', array($Encuesta->id), array('class' => 'btn btn-info', 'style'=>'width:160px; height:160px; padding-top: 8%')) }}
    {{ Form::open(array('method' => 'POST', 'route' => array('Fin.Pagos', $Encuesta->id), 'class' => 'col-md-3 center-text', 'style'=>'padding-top: 2%; margin-top:18%; margin-left:23%')) }}
    ...
    @if ($Encuesta->activa == 1)
      {{ Form::open(array('method' => 'POST', 'route' => array('Encuestas.desactivar', $Encuesta->id), 'class' => 'col-md-3 center-text', 'style'=>'; margin-top:18%; padding-top: 2%')) }}
